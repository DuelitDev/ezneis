...
def get_school(
    key: str, name: str = "", region: Optional[Region] = None, **kwargs
) -> SyncSchoolData:
...
async def get_school_async(
    key: str, name: str = "", region: Optional[Region] = None, **kwargs
) -> AsyncSchoolData:
    wrapper = _session_manager.get_async_wrapper(key)
    info = await wrapper.get_school_info(**kwargs, code=name, region=region, hint=1)
    if not info:
        raise DataNotFoundException
    return AsyncSchoolData(wrapper, info=info[0])


async def get_schools_async(
    key: str, name: str = "", region: Optional[Region] = None, **kwargs
