...
class SyncSchoolData:
    ...
    _schedules: Optional[SchedulesTuple] = field(default=None, init=False)
    ...
    _lecture_rooms: Optional[LectureRoomsTuple] = field(default=None, init=False)

    _timetables: Optional[TimetablesTuple] = field(default=None, init=False)

    _departments: Optional[DepartmentsTuple] = field(default=None, init=False)

    _majors: Optional[MajorsTuple] = field(default=None, init=False)
    ...
    def load_schedules(
        self, reload=False, *, date: Optional[str | tuple[str, str]] = None, **kwargs
    ):
        if self._schedules is not None and not reload:
            return
        self._schedules = SchedulesTuple(
            self._wrapper.get_schedules(
                **kwargs, code=self._code, region=self._region, date=date
            )
        )
    ...
    def load_lecture_rooms(
        ...
        **kwargs
        ...
        self._lecture_rooms = LectureRoomsTuple(
            self._wrapper.get_lecture_rooms(
                **kwargs,
                code=self._code,
                region=self._region,
                year=year,
    ...
    def load_majors(self, reload=False, **kwargs):
        ...
        self._majors = MajorsTuple(
            self._wrapper.get_majors(**kwargs, code=self._code, region=self._region)
        )
