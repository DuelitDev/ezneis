...
class AsyncSchoolData:
    ...
    async def load_schedules(
        ...
        self._schedules = SchedulesTuple(
            await self._wrapper.get_schedules(
    ...
    async def load_meals(
        ...
        self._meals = MealsTuple(
            await self._wrapper.get_meals(
                **kwargs, code=self._code, region=self._region, date=date
            )
        )

    async def load_classrooms(
        self,
        reload=False,
        *,
        ...
        **kwargs
        ...
        self._classrooms = ClassroomsTuple(
            await self._wrapper.get_classrooms(
                **kwargs,
                code=self._code,
                region=self._region,
                year=year,
    ...
    async def load_timetable(
        ...
        self._timetables = TimetablesTuple(
            await self._wrapper.get_timetables(
                **kwargs,
                code=self._code,
                region=self._region,
    ...
    async def load_majors(self, reload=False, **kwargs):
        ...
        self._majors = MajorsTuple(
            await self._wrapper.get_majors(
                **kwargs, code=self._code, region=self._region
            )
        )
